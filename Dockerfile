...
RUN apk add --no-cache freetype libpng libjpeg-turbo ca-certificates openssl libpng-dev libjpeg-turbo-dev freetype-dev icu-dev zip libzip-dev postgresql-dev
RUN docker-php-ext-configure gd --with-freetype --with-jpeg
RUN docker-php-ext-install gd pdo mysqli pdo_mysql pgsql pdo_pgsql intl zip
RUN wget https://github.com/fisharebest/webtrees/releases/download/$WEBTREES_VERSION/webtrees-$WEBTREES_VERSION.zip
RUN unzip webtrees-$WEBTREES_VERSION.zip -d /var/www/html
RUN mv /var/www/html/webtrees/* /var/www/html/
RUN rm -rf /var/www/html/webtrees
RUN rm webtrees-$WEBTREES_VERSION.zip
